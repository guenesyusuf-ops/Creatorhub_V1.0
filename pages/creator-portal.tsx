...
export default function CreatorPortalPage() {
  ...
  useEffect(() => {
    ...
    // Run JS - but init as creator portal directly
    try {
      const fn = new Function(JS)
      fn()
      // Open portal directly for this creator
      if (creator) {
        const cid = creator.id
        // Find creator in S and open portal
        setTimeout(() => {
          const w = window as any
          if (w.openPortal && w.S) {
            // Add creator to S if not exists
            if (!w.S.creators.find((c: any) => c.id === cid)) {
              w.S.creators.push({
                id: cid,
                name: creator.name,
                ini: creator.initials || creator.name?.slice(0,2)?.toUpperCase() || 'CR',
                color: '#7c3aed',
                email: creator.email || '',
                tags: [],
