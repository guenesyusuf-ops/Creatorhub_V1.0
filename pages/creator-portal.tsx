...
// Symbols to expose on window after APP_JS runs
...
window.rCT = typeof rCT !== 'undefined' ? rCT : null;
window.rCHdr = typeof rCHdr !== 'undefined' ? rCHdr : null;
window.rDash = typeof rDash !== 'undefined' ? rDash : null;
window.rCreators = typeof rCreators !== 'undefined' ? rCreators : null;
window.rCInvite = typeof rCInvite !== 'undefined' ? rCInvite : null;
...
export default function CreatorPortalPage() {
  ...
  useEffect(() => {
    function runAuth() {
      ...
      if (urlCode) {
        ...
        } catch { clearSession(); setAuthState('error') }
      } else {
        setAuthState('error')
      }
    }
  ...
  useEffect(() => {
    ...
    // Execute APP_JS with explicit window exports
    ...
    try {
      ...
      console.warn('[CreatorPortal] APP_JS warning:', e instanceof Error ? (e as Error).message : String(e))
    }

    const w = window as any

    // Install null-safe G() AFTER APP_JS ran
    ...
    // Wait for window.S and window.openPortal then open portal
    function waitAndOpen(attempts: number) {
      if (!w.S || !w.openPortal) {
        if (attempts > 40) {
          console.error('[CreatorPortal] Portal functions not available after', attempts, 'attempts')
          setAuthState('portal_error')
          return
        }
        requestAnimationFrame(() => waitAndOpen(attempts + 1))
        return
      }
      ...
      if (!w.S.creators.find((c: any) => String(c.id) === String(cid))) {
        w.S.creators.push({
          id: cid, name: creatorData.name || 'Creator',
          ini: (creatorData.initials || creatorData.name?.slice(0,2) || 'CR').toUpperCase(),
          color: '#7c3aed', email: creatorData.email || '',
          tags: [], flds: { bilder: [], videos: [], roh: [], auswertung: [] },
          notizenCreator: '', invited: true
        })
      }

      try { w.openPortal(cid) } catch(e) {
        console.error('[CreatorPortal] openPortal error:', e)
      }
      ...
      {authState === 'portal_error' && (
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui', padding: 24 }}>
          <div style={{ textAlign: 'center', maxWidth: 400 }}>
            ...
            <button onClick={() => window.location.reload()} style={{ marginTop: 16, padding: '12px 24px', background: '#111', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer' }}>
              Neu laden
            </button>
          </div>
        </div>
