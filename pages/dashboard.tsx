// v1.8 – Admin Dashboard mit Notification-Banner + Creator-Uploads
import { useState, useEffect } from 'react'
import { useRouter } from 'next/router'
import Head from 'next/head'

type View = 'main' | 'dates' | 'links' | 'uploads'
...
export default function Dashboard() {
  ...
  const [uploadsTab, setUploadsTab] = useState<string>('bilder')
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => loadNotifications(t), 30000)
  ...
  async function loadCreatorUploads(creatorId: string) {
    try {
      const res = await fetch(`/api/uploads?creator_id=${creatorId}`, { headers: { Authorization: `Bearer ${token()}` } })
      const data = await res.json()
      if (Array.isArray(data)) setCreatorUploads(data)
    } catch {}
  }
  ...
  async function deleteUpload(uploadId: string) {
    ...
    if (res.ok) {
      setCreatorUploads(prev => prev.filter(u => u.id !== uploadId))
  ...
  function navigate(p: Page) {
    setPage(p); setView('main'); setSearch('')
    if (p === 'team') loadTeam()
  }
  ...
  async function confirmModal() {
    if (modal === 'addCat') {
      ...
      setCategories(prev => [...prev, d]); setActiveCat(d.id)
      ...
      await fetch('/api/dates', { method: 'POST', headers: authHeader(), body: JSON.stringify({ creator_id: activeCreator.id, month: form.month }) })
      await loadDates(activeCreator.id)
      showToast('Datum hinzugefügt ✓')
    } else if (modal === 'addLink') {
      await fetch('/api/links', { method: 'POST', headers: authHeader(), body: JSON.stringify({ date_id: activeDate.id, creator_id: activeCreator.id, title: form.linkTitle, url: form.linkUrl }) })
  ...
  function formatDate(str: string) {
    return new Date(str).toLocaleDateString('de-DE', { day: '2-digit', month: '2-digit', year: '2-digit', hour: '2-digit', minute: '2-digit' })
  }
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 10, alignItems: 'center' }}>
              ...
              {notifications.length > 0 && (
                <div style={{ position: 'relative' }}>
                  <div style={{ width: 8, height: 8, background: '#c8f035', borderRadius: '50%', position: 'absolute', top: -2, right: -2, zIndex: 1 }} />
          ...
          {notifications.length > 0 && (
            <div style={{ padding: '12px 32px 0', display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                <div style={{ fontSize: 12, color: '#c8f035', fontWeight: 600 }}>🔔 {notifications.length} neue Benachrichtigung{notifications.length > 1 ? 'en' : ''}</div>
                <button onClick={dismissAllNotifications} style={{ background: 'none', border: 'none', color: '#6b6b80', fontSize: 11, cursor: 'pointer', textDecoration: 'underline' }}>
                  Alle schließen
                </button>
              </div>
              {notifications.map(n => (
                <div key={n.id} style={{ background: '#13131c', border: '1px solid rgba(200,240,53,0.2)', borderLeft: '3px solid #c8f035', borderRadius: 10, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: 18 }}>📤</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, color: '#f0f0f5' }}>{n.message}</div>
                    <div style={{ fontSize: 11, color: '#6b6b80', marginTop: 2 }}>{formatDate(n.created_at)}</div>
                  </div>
                  <button onClick={() => dismissNotification(n.id)}
                    style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 6, width: 26, height: 26, cursor: 'pointer', color: '#6b6b80', fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ✕
                  </button>
                </div>
              ))}
              ...
              <>
                  ...
                  <>
                    ...
                    <div style={st.creatorGrid}>
                      {filteredCreators.map(c => {
                        ...
                        const hasNewUpload = notifications.some(n => n.creator_id === c.id)
                        return (
                          <div key={c.id} style={{ ...st.creatorCard, position: 'relative' as const }} onClick={() => openCreator(c)}>
                            {hasNewUpload && (
                              <div style={{ position: 'absolute', top: 10, right: 10, width: 10, height: 10, background: '#c8f035', borderRadius: '50%', boxShadow: '0 0 6px #c8f035' }} title="Neuer Upload" />
                            )}
                ...
                {view === 'dates' && activeCreator && (
                  <>
                    ...
                    <div style={st.pageHeader}>
                      <div><div style={st.pageTitle}>{activeCreator.name} <span style={{ color: '#c8f035' }}>Inhalte</span></div></div>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
                      {[
                        ...
                        { id: 'uploads', label: `📁 Dateien (${creatorUploads.length})` },
                      ].map(t => (
                        <button key={t.id} onClick={() => setView(t.id as View)}
                          style={{ padding: '8px 18px', borderRadius: 20, border: view === t.id ? '1px solid #c8f035' : '1px solid rgba(255,255,255,0.07)', background: view === t.id ? '#c8f035' : '#111118', color: view === t.id ? '#0a0a0f' : '#6b6b80', fontSize: 13, cursor: 'pointer', fontWeight: view === t.id ? 700 : 400 }}>
                          {t.label}
                        </button>
                      ))}
                    </div>
                ...
                {/* UPLOADS VIEW */}
                {view === 'uploads' && activeCreator && (
                  <>
                    <button style={st.backBtn} onClick={() => setView('dates')}>← Zurück zu Dates</button>
                    <div style={st.breadcrumb}><span>Dashboard</span><span style={st.sep}>›</span><span>{activeCreator.name}</span><span style={st.sep}>›</span><span style={st.bcCurrent}>Dateien</span></div>
                    <div style={st.pageHeader}>
                      <div><div style={st.pageTitle}>{activeCreator.name} <span style={{ color: '#c8f035' }}>Uploads</span></div></div>
                    </div>

                    {/* Tab filter */}
                    <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' as const }}>
                      {uploadTabs.map(t => (
                        <button key={t.key} onClick={() => setUploadsTab(t.key)}
                          style={{ padding: '8px 16px', borderRadius: 20, border: uploadsTab === t.key ? '1px solid #c8f035' : '1px solid rgba(255,255,255,0.07)', background: uploadsTab === t.key ? '#c8f035' : '#111118', color: uploadsTab === t.key ? '#0a0a0f' : '#6b6b80', fontSize: 12, cursor: 'pointer' }}>
                          {t.icon} {t.label} ({creatorUploads.filter(u => u.category === t.key).length})
                        </button>
                      ))}
                    </div>

                    {creatorUploads.filter(u => u.category === uploadsTab).length === 0 ? (
                      <div style={{ color: '#6b6b80', fontSize: 14, padding: 24, background: '#13131c', borderRadius: 16, border: '1px solid rgba(255,255,255,0.07)', textAlign: 'center' as const }}>
                        Keine Dateien in dieser Kategorie.
                      </div>
                    ) : (
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 14 }}>
                        {creatorUploads.filter(u => u.category === uploadsTab).map(u => (
                          <div key={u.id} style={{ background: '#13131c', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden' }}>
                            {/* Thumbnail */}
                            <div style={{ height: 130, background: '#1a1a24', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', fontSize: 36 }}>
                              {u.mime_type?.startsWith('image/') ? (
                                <img src={u.file_url} alt={u.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                              ) : u.mime_type?.startsWith('video/') ? '🎬' : '📄'}
                            </div>
                            <div style={{ padding: '10px 12px' }}>
                              <div style={{ fontSize: 12, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const, color: '#f0f0f5' }}>{u.name}</div>
                              {u.batch && <div style={{ fontSize: 10, color: '#c8f035', marginTop: 3 }}>📦 {u.batch}</div>}
                              {u.product && <div style={{ fontSize: 10, color: '#a0a0b8', marginTop: 2 }}>🏷️ {u.product}</div>}
                              <div style={{ fontSize: 10, color: '#6b6b80', marginTop: 3 }}>{formatDate(u.created_at)}{u.file_size ? ` · ${formatSize(u.file_size)}` : ''}</div>
                              <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                                <a href={u.file_url} target="_blank" rel="noreferrer"
                                  style={{ flex: 1, background: '#1a1a24', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 7, padding: '5px 0', fontSize: 11, color: '#a0a0b8', textDecoration: 'none', textAlign: 'center' as const }}>
                                  Öffnen ↗
                                </a>
                                {user?.role === 'admin' && (
                                  <button onClick={() => deleteUpload(u.id)}
                                    style={{ background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 7, padding: '5px 8px', fontSize: 11, color: '#ef4444', cursor: 'pointer' }}>
                                    🗑
                                  </button>
                                )}
                              </div>
      ...
      {modal && (
        <div style={st.overlay} onClick={e => { if (e.target === e.currentTarget) setModal('') }}>
          <div style={st.modal}>
            ...
            {modal === 'addCat' && (<><MField label="Name" value={form.catName} onChange={(v: string) => setForm({ ...form, catName: v })} placeholder="z.B. Reels, Storys..." /><MField label="Emoji" value={form.catIcon} onChange={(v: string) => setForm({ ...form, catIcon: v })} placeholder="🎥" /></>)}
            {modal === 'addCreator' && (<><MField label="Name" value={form.creatorName} onChange={(v: string) => setForm({ ...form, creatorName: v })} placeholder="z.B. Anna M." /><MField label="Kürzel (2-3 Buchst.)" value={form.creatorInitials} onChange={(v: string) => setForm({ ...form, creatorInitials: v })} placeholder="AM" /><div style={st.fGroup}><label style={st.fLabel}>Kategorie</label><select style={st.fInput} value={form.creatorCat || activeCat} onChange={e => setForm({ ...form, creatorCat: e.target.value })}>{categories.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}</select></div></>)}
            {modal === 'addDate' && <MField label="Monat & Jahr" type="month" value={form.month} onChange={(v: string) => setForm({ ...form, month: v })} />}
            {modal === 'addLink' && (<><MField label="Bezeichnung" value={form.linkTitle} onChange={(v: string) => setForm({ ...form, linkTitle: v })} placeholder="z.B. Brand Video März" /><MField label="URL" value={form.linkUrl} onChange={(v: string) => setForm({ ...form, linkUrl: v })} placeholder="https://..." /></>)}
            {modal === 'invite' && (<><MField label="Name" value={form.invName} onChange={(v: string) => setForm({ ...form, invName: v })} placeholder="Max Mustermann" /><MField label="E-Mail" type="email" value={form.invEmail} onChange={(v: string) => setForm({ ...form, invEmail: v })} placeholder="max@company.com" /><div style={st.fGroup}><label style={st.fLabel}>Zugriffsrecht</label><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 6 }}>{[{ val: 'read', icon: '👁', label: 'Lesen', desc: 'Kann Inhalte nur ansehen' }, { val: 'admin', icon: '⚡', label: 'Admin', desc: 'Vollzugriff & Verwaltung' }].map(r => (<div key={r.val} onClick={() => setSelectedRole(r.val)} style={{ padding: 14, borderRadius: 12, border: selectedRole === r.val ? '2px solid #c8f035' : '2px solid rgba(255,255,255,0.07)', background: selectedRole === r.val ? 'rgba(200,240,53,0.06)' : 'transparent', cursor: 'pointer' }}><div style={{ fontFamily: 'Syne, sans-serif', fontSize: 14, fontWeight: 700, marginBottom: 4 }}>{r.icon} {r.label}</div><div style={{ fontSize: 12, color: '#6b6b80' }}>{r.desc}</div></div>))}</div></div></>)}
            {modal === 'inviteCreator' && <MField label="E-Mail der Creatorin" type="email" value={form.invCreatorEmail} onChange={(v: string) => setForm({ ...form, invCreatorEmail: v })} placeholder="creator@email.com" />}
