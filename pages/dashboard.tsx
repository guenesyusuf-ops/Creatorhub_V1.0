...
<div id="cw-bg" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);backdrop-filter:blur(6px);z-index:800;overflow-y:auto;padding:24px 16px">
  <div style="background:var(--surf);border-radius:20px;max-width:820px;margin:0 auto;padding:28px 24px;position:relative">
    <button id="cw-close" style="position:absolute;top:14px;right:16px;background:none;border:none;font-size:20px;cursor:pointer;color:var(--muted)">✕</button>
...
G('tool-headline')?.addEventListener('click',function(){
  ...
  h+='<textarea id="cw-produkt" rows="3" style="'+fi+';resize:vertical;margin-bottom:10px" placeholder="z.B. 3D Stift mit magischer Tinte..."></textarea>';
  h+='<div style="'+g3+'">';
  ...
  for(var i=0;i<hooks.length;i++){
    ...
    h+='<input type="checkbox" class="cw-hook" value="'+hooks[i]+'" style="width:14px;height:14px"> '+icos[i]+' '+hooks[i];
    h+='</label>';
  }
  h+='</div></div>';
  h+='<div style="'+sc+'">';
...
document.addEventListener('click',function(e){
  ...
  if(t===G('cw-bg')||t.id==='cw-close'){G('cw-bg').style.display='none';document.body.style.overflow='';}
  if(t.id==='cw-copy-btn'){navigator.clipboard.writeText(G('cw-output')?.textContent||'').then(function(){showT('✓ Kopiert');});}
});
document.addEventListener('click',function(e){
  ...
  G('cw-result').style.display='none';
  var hooks=Array.from(document.querySelectorAll('.cw-hook:checked')).map(function(c){return (c as HTMLInputElement).value;});
  var body={
    produkt:(G('cw-produkt') as HTMLTextAreaElement)?.value||'',
    geschlecht:(G('cw-gschl') as HTMLSelectElement)?.value||'',
  ...
  fetch('/api/copywriter',{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+(localStorage.getItem('token')||'')},body:JSON.stringify(body)})
    .then(function(r){return r.json();}).then(function(data){
      btn.disabled=false;btn.textContent='⚡ Headlines & Copy generieren';
