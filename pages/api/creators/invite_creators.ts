...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (findError) {
    console.warn(`[INVITE] ${ts} | Find error:`, findError.message)
  }

  // Keep existing code if creator already has one (preserve magic link)
  // Only generate new code if no code exists yet
  const code = existing?.invite_code || generateInviteCode()

  console.log(`[INVITE] ${ts} | Start | email=${email} | code=${code} | existing=${!!existing?.invite_code}`)
